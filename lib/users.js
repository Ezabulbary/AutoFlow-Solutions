...
// Find-or-create for Google OAuth sign-in. Uses an atomic update for the
// existing-user path so we never re-validate a whole legacy document.
export async function findOrCreateGoogleUser({ email, name, googleId, role = 'USER' }) {
  ...
  if (existing) {
    const set = {};
    if (!existing.googleId) set.googleId = googleId;
    if (existing.provider === 'local') set.provider = 'google';
    if (Object.keys(set).length) {
      await User.updateOne({ _id: existing._id }, { $set: set });
    }
    return full({ ...existing, ...set });
  }
